...
if settings.Config.AnalyzeMode:
	print ('[i] Responder is in analyze mode. No NBT-NS, LLMNR, MDNS requests will be poisoned.', 3, 1)
...
def serve_thread_udp_broadcast(host, port, handler):
	try:
		...
		print ("[!] ", 1, 1) + "Error starting UDP server on port " + str(port) + ", check permissions or other servers running."
...
def serve_thread_tcp(host, port, handler):
	try:
		...
		print ("[!] ", 1, 1) + "Error starting TCP server on port " + str(port) + ", check permissions or other servers running."
...
def serve_thread_SSL(host, port, handler):
	try:
		...
		print ("[!] ", 1, 1) + "Error starting SSL server on port " + str(port) + ", check permissions or other servers running."
...
def main():
	try:
		...
		print ('[+]', 2, 1) + " Listening for events..."
